...
The CI job `bindings-unity-il2cpp` (`.github/workflows/ci.yml`) codifies this exact sequence and has now passed a real run in GitHub Actions on `windows-latest`, with `UNITY_EMAIL`/`UNITY_PASSWORD` repository secrets for a Unity Personal license — not just locally. This closes the risk register's P1 "definition of done" requirement for Phase 3. No caching configured yet for the Unity Hub/Editor/IL2CPP download, so this job is slow (~25 minutes, mostly download) — a real follow-up, not blocking correctness.
